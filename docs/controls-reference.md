...
| `Dark` | 어두운 배경 전체 반전 |
| `Gray` | 라이트 기반, 무채색 슬레이트 액센트 `#64748B` + 쿨 그레이 배경 톤 |
| `Purple` | 라이트 기반, 바이올렛 액센트 `#7C3AED` + 라벤더 배경 톤 |
| `Orange` | 라이트 기반, 오렌지 액센트 `#EA580C` + 웜 배경 톤 |
| `Tomato` | 라이트 기반, 토마토 액센트 `#D9442B` + 웜 레드 배경 톤 |

틴트 테마 4종(Gray/Purple/Orange/Tomato)은 **라이트 기반**이다: 표면은 흰색,
텍스트·시맨틱(성공/경고/오류) 색은 라이트와 공유하고, 액센트 계열과 배경 톤만
바뀐다. 적용 방법은 다크와 완전히 동일하다 (`Mode` 설정 + 화면당 `Apply(this)`).

| 멤버 | 설명 |
|---|---|
| `ModernTheme.Mode` | 위 6가지 중 하나. 시작 시 한 번만 설정 |
| `ModernTheme.IsDark` | 현재 다크 여부 (읽기 전용; 틴트 테마는 false) |
