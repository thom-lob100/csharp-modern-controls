...
Common에 포함된 공개 `GridCaptionDictionary`는 회사 표준 "필드 이름 → 캡션" 용어집을
제공한다. 앱 시작 시(첫 폼 생성 전) 등록하면, 캡션 인자가 없는 생성자가 사전을 참조한다:

```csharp
// Program.Main — 앱당 1회. Common만 참조하는 앱도 그대로 사용 가능
Modern.Lab.Services.GridCaptionDictionary.RegisterAll();
...
| `SetPreviewMarkers(Dictionary<string,string>)` | LCC 미리보기의 자리 키 → 삽입 위치(`Top`/`Left`/`Right`) 맵. `SetPreview` 뒤에 설정하면 비어 있는 대상 핑거에도 **옅은(반투명) 2px 액센트 틱**으로 방향을 표시한다 — 실제 수납 틱보다 존재감을 낮춰 빈 자리 위에서 과하게 튀지 않게 한다 (null = 해제) |
