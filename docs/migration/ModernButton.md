...
| `GlyphSize` | 캡션/아이콘 글자 크기 재정의(px, 0 = 토큰 기본). 화살표·기호 같은 아이콘형 캡션을 크게 보일 때 사용 — 값이 있으면 좁은 아이콘 버튼에서 글리프가 잘리지 않도록 좌우 패딩이 자동으로 줄어든다 |
| `TopLabel` | 글리프/캡션 **위**에 붙는 아주 작은 상단 라벨(비우면 숨김). 아이콘형 버튼에 "All"/"Selected" 같은 부가 설명을 얹을 때 사용 — 값이 있으면 두 줄이 잘리지 않게 버튼 높이를 내용에 맞춰 늘린다 |
...
| `Subtle` | 테두리 없음, 텍스트만 (hover 시 회색 채움) | 저강조 동작 (초기화 등) |
| `Excel` | 흰 배경 + 초록 글자·외곽선 (hover 시 옅은 초록 채움) | 엑셀 내보내기 전용 포인트. 색은 `Brush.Excel*` 토큰이라 테마별로 다르게 정의된다 (다크 테마는 밝은 초록) |
