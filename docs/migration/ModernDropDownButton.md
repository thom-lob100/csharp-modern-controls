...
| `DataSource` | 메뉴 항목: `DataTable`/`DataView`/`IList`/`IEnumerable`. 표시 텍스트가 `"-"`인 행은 **구분선**(클릭 불가) — `ToolStripSeparator` 관례 |
...
| `Kind` | 버튼 시각 종류 — `Secondary`(기본) / `Execute`(초록 채움) / `Excel`(초록 아웃라인; 엑셀 내보내기 포인트) — `ModernButton.Kind`와 같은 문법. 캡션 굵기도 `ModernButton`과 동일한 일반(Normal) 굵기 |
| `IsDropDownOpen` | 읽기 전용. 메뉴 팝업이 열려 있으면 true — 자동 갱신/재바인딩 화면은 true 동안 갱신을 보류해 사용 중인 메뉴를 유지 |
