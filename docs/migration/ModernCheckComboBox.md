...
| `CheckAll()` / `UncheckAll()` | 전체 체크/해제 (이벤트 1회 발생) — 드롭다운 상단 "(All)" 헤더와 동일 동작 |
...
- 필드 클릭 → 체크 목록 드롭다운. 체크해도 닫히지 않아 여러 개 선택 가능,
  바깥 클릭 시 닫힘
- 드롭다운 상단 **"(All)" 헤더**(그리드 필터 팝업과 동일 형식): 데이터 항목이
  아니라 전체 선택/해제 **헬퍼**다 — 전부 체크 = 체크, 일부 = 부분 선택(대시),
  없음 = 해제. 클릭 시 전체 체크 ↔ 전체 해제 토글.
- **전체 선택 시 필드는 값을 나열하지 않고 placeholder로 접힌다** — 조회
  조건에서 전체 선택과 빈 선택은 둘 다 "조건 없음"으로 같은 의미다(관례).
