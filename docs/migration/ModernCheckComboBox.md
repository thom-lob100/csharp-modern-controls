...
| `CheckAll()` / `UncheckAll()` | 전체 체크/해제 (이벤트 1회 발생) — 드롭다운 상단 "Select all" 헤더와 동일 동작 |
...
- 필드 클릭 → 체크 목록 드롭다운. 체크해도 닫히지 않아 여러 개 선택 가능,
  바깥 클릭 시 닫힘
- 드롭다운 상단 **전체 선택/해제 헤더**: 전부 체크 = 체크 표시 + 라벨 "Deselect all",
  일부만 = 부분 선택(대시), 없음 = 해제(라벨 "Select all") — 클릭 시 전체 체크 ↔ 전체 해제 토글
